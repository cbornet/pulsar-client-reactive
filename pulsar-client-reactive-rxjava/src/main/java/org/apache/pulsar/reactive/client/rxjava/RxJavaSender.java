...
/**
 * An RxJava wrapper for a {@link ReactiveMessageSender}.
 */
public class RxJavaSender<T> {

	private final ReactiveMessageSender<T> sender;

	/**
	 * Create a new {@link RxJavaSender} wrapping the given {@link ReactiveMessageSender}.
	 * @param sender the sender to wrap
	 */
	public RxJavaSender(ReactiveMessageSender<T> sender) {
		this.sender = sender;
	}
	 ...
	 */
	public Single<MessageId> sendOne(MessageSpec<T> messageSpec) {
		return Single.fromPublisher(this.sender.sendOne(messageSpec));
	}
	 ...
	 */
	public Flowable<MessageSendResult<T>> sendMany(Publisher<MessageSpec<T>> messageSpecs) {
		return Flowable.fromPublisher(this.sender.sendMany(messageSpecs));
	}
