...
/**
 * Tests for {@link RxJavaSender}
 */
class RxJavaSenderTest {
	...
	void sendOne() {
		...
		RxJavaSender<String> rxJavaSender = new RxJavaSender<>(sender);
		rxJavaSender.sendOne(MessageSpec.builder("test").correlationMetadata("test").build()).test()
				.assertResult(MessageId.earliest).assertComplete().awaitDone(5, TimeUnit.SECONDS);
	}
	...
	void sendMany() {
		...
		rxJavaSender.sendMany(messageSpecs).map(MessageSendResult::getMessageSpec).test().assertComplete()
				.assertResult(message1, message2).awaitDone(5, TimeUnit.SECONDS);
	}
